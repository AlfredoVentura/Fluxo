# Especificação de Requisitos — Sistema Bancário Digital
...
| **RF03** | O sistema deve submeter o cadastro a uma verificação de identidade (KYC) simulada e automatizada antes de liberar a conta. | Must | UC02 |
| **RF04** | O sistema deve aprovar ou reprovar a abertura de conta automaticamente com base na simulação, registrando o motivo em caso de recusa. | Must | UC06 |
...
| **RF08** | O sistema deve bloquear o acesso após 5 tentativas de login incorretas em 15 minutos, exigindo que o usuário solicite desbloqueio via suporte. | Must | UC03 |
...
| **RF17** | O sistema deve impedir o registro de uma chave Pix já vinculada a outra conta de forma rigorosa. | Must | UC10 |
...
| **RF22** | O sistema deve permitir agendar transferências para data futura, validando o saldo estritamente no dia da execução e disparando e-mail de falha em caso de insuficiência. | Should | UC14 |
...
| **RF48** | O sistema deve exibir requisições pendentes de desbloqueio de contas bloqueadas por tentativas de login na visão de administração/suporte para liberação direta. | Must | UC33 |
| **RF49** | O sistema deve permitir ao auditor consultar a trilha com filtros por autor, período e tipo de operação. | Should | UC34 |
| **RF50** | O sistema deve permitir parametrizar tarifas, limites por operação, limites diários e o teto máximo de segurança para ajuste dos clientes. | Should | UC35 |
| **RF51** | O sistema deve gerar relatórios gerenciais de volume transacionado, contas ativas e chamados. | Should | UC36 |
| **RF52** | O sistema deve permitir ao cliente abrir chamado de suporte com assunto, descrição e anexo. | Should | UC38 |
| **RF53** | O sistema deve permitir ao analista responder o chamado e alterar seu status. | Should | UC37 |

> **Resumo Funcional:** 53 requisitos funcionais.
...
| ID | Regra de Negócio |
| :--- | :--- |
| **RN01** | Uma conta só é ativada após aprovação automatizada do KYC pelo sistema. |
...
| **RN07** | Entre 20h e 6h, o limite por operação cai para R$ 1.000,00 (limite noturno), sendo permitida a alteração dinâmica pelo próprio usuário dentro do teto de segurança global. |
| **RN08** | Um cliente pode ter no máximo 5 chaves Pix, sendo no máximo uma por tipo (exceto aleatória) e com recusa estrita de duplicidade. |
...
---
> **Observação:** Versão consolidada e atualizada (Checkpoint 1).
